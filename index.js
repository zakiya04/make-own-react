...
//we compare the old fiber with the new one and change accordingly//
...
function updateDom(dom, prevProps, nextProps){
  ...
  //change eventlisteners or remove them//
  ...
  //add new property to elements//
  Object.keys(nextProps).filter(isProperty).filter(isNew(prevProps,nextProps)).forEach(name =>{
    dom[name] = nextProps[name]
  })

  //at last add the new event listeners//
  Object.props(nextProps).filter(isEvent).filter(isNew(prevProps,nextProps).forEach(name =>{
    const eventType = name.toLowerCase().substring(2);
